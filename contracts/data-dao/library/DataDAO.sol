...
contract DataDAO is IDataDAO, DataDAOCore, AccessControl {
    ...
    function activateDeal(uint64 _networkDealID) internal returns(uint64) {
        ...
        uint64 provider = MarketAPI.getDealProvider(_networkDealID);

        authorizeDealData(commitmentRet.data, provider, commitmentRet.size);

        uint64 client = MarketAPI.getDealClient(_networkDealID);
        // Activate the deal
